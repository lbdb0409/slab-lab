...
export function AccountSidebar({ name, email }: { name: string; email: string }) {
  ...
  return (
    <aside className="md:sticky md:top-[180px] md:self-start">
      <div className="flex items-center gap-3 border-2 border-text bg-white p-5">
      ...
      <nav className="mt-3 flex flex-col">
        {TABS.map((tab) => {
          ...
          return (
            <Link
              ...
              href={tab.href}
              className={cn(
                "flex items-center gap-3 border-2 border-text bg-white px-4 py-3 -mt-px text-sm font-bold uppercase tracking-wider transition-colors",
                active
        ...
        <button
          type="button"
          className="-mt-px flex items-center gap-3 border-2 border-line bg-white px-4 py-3 text-sm font-bold uppercase tracking-wider text-muted transition-colors hover:bg-bg-soft hover:text-orange"
        >
